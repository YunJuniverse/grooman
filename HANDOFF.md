...
- Working on: **GRM-013 측정 인프라** — GTM 설치 + 개인정보처리방침 정비 완료(리뷰 대기)
- Current mode: fullstack
- Next TODO: 이 PR 리뷰 → **Vercel env 설정(인간)**: Supabase 자격증명 + `NEXT_PUBLIC_GTM_ID`(Production만) → GTM 콘솔에서 GA4 연결(인간) → GRM-001 숫자측정
- Blockers: Vercel env·GA4/Search Console 콘솔 작업 = 인간 액션 선행 필요

## Active Links

- Current PR: branch feat/grm-013-analytics(신규) · 직전 #14 머지됨
- Current issue:
...
- 2026-07-22: **GRM-013 GTM 설치** — `@next/third-parties`+noscript, `NEXT_PUBLIC_GTM_ID` 게이트(preview 오염 차단) · 개인정보처리방침 처리위탁·분석쿠키·정보주체권리 신설 · Class A
